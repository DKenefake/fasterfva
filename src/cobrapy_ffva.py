...
def faster_fva(
    ...
    def remove_bound_problems(v_vals, lower_bounds, upper_bounds):
        ...
        lbp_to_remove = np.where((v_vals <= lower_bounds + tol) & (v_vals >= lower_bounds - tol))[0]
        lower_bound_problems.difference_update(set(lbp_to_remove))
        if len(lbp_to_remove) > 0:
            fva_result.loc[np.array(reaction_ids)[lbp_to_remove], 'minimum'] = lower_bounds[lbp_to_remove]

        ubp_to_remove = np.where((v_vals <= upper_bounds + tol) & (v_vals >= upper_bounds - tol))[0]
        upper_bound_problems.difference_update(set(ubp_to_remove))
        if len(ubp_to_remove) > 0:
            fva_result.loc[np.array(reaction_ids)[ubp_to_remove], 'maximum'] = upper_bounds[ubp_to_remove]
    ...
    with model:
        ...
        v_vals = np.array([r.flux for r in model.reactions])
        lower_bounds = np.array([r.lower_bound for r in model.reactions])
        upper_bounds = np.array([r.upper_bound for r in model.reactions])
        ...
        while len(upper_bound_problems) != 0:
            ...
            upper_bounds = np.array([r.upper_bound for r in model.reactions])
            remove_bound_problems(v_vals, lower_bounds, upper_bounds)
    print('{} LPs were solved during this analysis. The regular algorithm would have solved {} LPs.'.format(number_lps, 2*len(reaction_ids)+1))
    return fva_result[["minimum", "maximum"]]
